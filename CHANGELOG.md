...
**实测(诚实)**:21 负载 best-of-7,两侧均由 Cython 编译,`--cython` 相对原味 cythonize **0.93x geomean**——变慢,因为被关掉的推断正是若干行速度的来源(误编译的推导式比正确版快 1.55x)。标注覆盖也很低:52 个基准函数只有 2 个通过门槛。门槛之一是新发现的**部分标注负收益**:typed 值遇 untyped 操作数要装箱往返,实测 0.72x,故改为"函数内算术涉及的每个操作数都可证才标注,否则一个都不标"。天花板则很高:手工把某循环 5 个局部全标上是 **11.35x**——差距在区间分析覆盖(flow-insensitive 的 `while` 计数器会被加宽到无穷、attr-hoist 临时变量遮蔽 `range()` 形态、推导式变量无法用 `@cython.locals` 标注),不在机制。
