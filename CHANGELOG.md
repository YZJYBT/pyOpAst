...
**它修的是正确性,不是速度**:原味 `cythonize` 不保持语义,两处偏差已在本仓库基准语料复现——① `[j * j % 97 for j in range(0, 400000, 4)]` 末元素应为 54,原味 Cython 给 20(它给 `j` 推断 C `long`,MSVC 下 32 位,`399996²` 回绕);② 条件绑定的局部变量本应抛 `UnboundLocalError`,编译后返回 0。两者都源自 Cython 自己的"安全"推断,而它没有区间分析;opast 有(`j*j` 可证 `(0, 159996800016)`)。`--cython` 关掉该推断并补上可证类型后,两例均与纯 Python 逐字节一致。

**局部变量的注解也被信任**(`annotated_locals`,新增于 `analysis.py`):`x: int = compute()` / `x: float = ...` 与参数注解同等对待,但要求**该声明是这个名字的唯一绑定**(重绑后可能装着注解没描述过的东西,运行期无人复查)。关键在于信任必须**参与不动点**而不只覆盖被声明的名字——`acc = (acc + i * x) % M` 要先信 `x` 才证得出 `acc`,否则一个不透明的声明会经"全有或全无"规则把整个函数拖垮(实测:`d_checked` 明明已进 checked 模式仍一个类型都标不上)。配套改动:`infer_int_names` / `infer_float_names` 对**已被信任的名字**不再复查其 `AnnAssign` 的值——复查它等于不信任它;参数被普通赋值重绑仍照旧检查并可被拒。

**有注解的整数进 checked 模式**:`int` 注解说的是"是整数"不是"不大",推不出区间,所以注解参数单靠自己不会被标注。现在只要函数里有一个 `int` 注解参数(需开 `--aggressive=annotations`),整个函数切到 checked 模式——每个可证 int 都标注(不管有没有界),并加 `@cython.overflowcheck(True)`。依据是实测出的**响度不对称**:参数转换装不下 int64 时 Cython 已经会抛 `OverflowError`,但 C 整数之间的算术会静默回绕(typed 函数里 `4e9 * 4e9` 实测 `-2446744073709551616`);该指令让后者也抛,代价约 0.79x。于是契约是"这些值放得下 64 位,放不下给异常而不是错答案"——比 `--jit` 的静默回绕严格更响。

**修支配性判定过粗**:原先只认函数顶层的绑定点,导致 opast 自己在循环体里造的临时变量(CSE 结果、外提的不变量)一律不合格,进而被"全有或全无"规则连累整个函数。改为对任意语句块递归成立:一个名字要有**单个绑定点支配它的全部使用**(`if` 两分支各赋一次者不合格)。

**实测**:21 负载两侧均由 Cython 编译,best-of-15 **1.29x geomean / 中位 1.05x**。这不是普涨,是两个大赢:`annotated` **16.6x**(22.7→1.4ms)、`strength` **14.8x**(163.4→11.1ms),交错 best-of-13 复核、结果一致、按中位数亦为 17.0x / 11.7x。覆盖仍窄(基准语料 36 个函数中 4 个拿到类型),多数行 ~1.0x;**具体哪 4 个取决于选项集**——pass 跑在最后,看到的是前面留下的形状:`-O2` 下含 `fastmath.work`,裸 `--aggressive=annotations` 下换成 `tailrec.work`,不开激进项则只有 2 个。**小数字不可信**:计时单位是"一个进程一次模块导入",Windows 首次接触扫描尖峰 best-of-N 压不干净,全量报 0.71x/0.72x 的两行单独重测是 1.04x/1.01x;只有上面那个量级的差异值得读。不依赖计时的:没拿到类型的行与 `-O2` 只差那道指令(指令本身 0.74–0.80x);部分标注比不标注更差(0.72x),故有"全有或全无"规则。门槛之一是新发现的**部分标注负收益**:typed 值遇 untyped 操作数要装箱往返,实测 0.72x,故改为"函数内算术涉及的每个操作数都可证才标注,否则一个都不标"。天花板则很高:手工把某循环 5 个局部全标上是 **11.35x**——差距在区间分析覆盖(flow-insensitive 的 `while` 计数器会被加宽到无穷、attr-hoist 临时变量遮蔽 `range()` 形态、推导式变量无法用 `@cython.locals` 标注),不在机制。
