...
**实测(诚实)**:21 负载两侧均由 Cython 编译,`--cython` 相对原味 cythonize best-of-7 为 **0.93x**、best-of-15 为 **0.97x** geomean(中位数均 0.98x)——**当区间读,不是测量值**:计时单位是"一个进程一次模块导入"(扩展模块无法在进程内重跑),Windows 起进程的首次接触扫描尖峰 best-of-N 压不干净,逐行在两轮间可差 ±0.4x。不依赖计时的部分:21 个负载有 **19 个一个类型都没拿到**,其与 `-O2` 的全部差别就是那道指令;单负载四版本对照(交错 11 轮)把两半分开计价——**指令 0.74–0.80x,类型在能用处 1.73x**;两轮最差行都是那个被误编译的推导式(0.59x / 0.49x),即算错的比算对的快一倍。标注覆盖很低:52 个基准函数只有 2 个通过门槛。门槛之一是新发现的**部分标注负收益**:typed 值遇 untyped 操作数要装箱往返,实测 0.72x,故改为"函数内算术涉及的每个操作数都可证才标注,否则一个都不标"。天花板则很高:手工把某循环 5 个局部全标上是 **11.35x**——差距在区间分析覆盖(flow-insensitive 的 `while` 计数器会被加宽到无穷、attr-hoist 临时变量遮蔽 `range()` 形态、推导式变量无法用 `@cython.locals` 标注),不在机制。
