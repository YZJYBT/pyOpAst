...
def assert_same_output(area: Area, path: Path, *args: str) -> None:
    ...
    optimized_cmd = [PYTHON, "-m", "pyopt", str(path)]
    ...
    if (a.returncode, a.stdout, a.stderr) != (b.returncode, b.stdout, b.stderr):
        area.fail(
            ...
            f"  pyopt    rc={b.returncode} stdout={b.stdout!r} stderr={b.stderr!r}",
...
def show(path: Path, *extra: str) -> subprocess.CompletedProcess[str]:
    return run([PYTHON, "-m", "pyopt", "--show", "--no-run", *extra, str(path)])
...
def verify_jit() -> Area:
    ...
    jit_run = run([PYTHON, "-m", "pyopt", "--jit", str(hot)], "20000")
    ...
    if (jit_run.returncode, jit_run.stdout) != (original.returncode, original.stdout):
        area.fail(
            ...
            command_text([PYTHON, "-m", "pyopt", "--jit", str(hot)], "20000"),
        )

    normal_show = show(hot)
    jit_show = show(hot, "--jit")
    if normal_show.returncode != 0 or "_pyopt_jit" in normal_show.stdout:
        area.fail("JIT helper/decorator appeared without --jit", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(hot)]))
    if jit_show.returncode != 0 or "import pyopt.jitsupport as _pyopt_jit" not in jit_show.stdout or "@_pyopt_jit.maybe_njit" not in jit_show.stdout:
        area.fail("--jit --show did not inject jitsupport import/decorator", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", "--jit", str(hot)]))

    report = run([PYTHON, "-m", "pyopt", "--jit", "--report", "--no-run", str(hot)])
    if report.returncode != 0 or "jit:" not in report.stderr:
        area.fail("--jit --report did not include jit pass stats", command_text([PYTHON, "-m", "pyopt", "--jit", "--report", "--no-run", str(hot)]))
    ...
    fallback = write_case(
        ...
        print(hot(12000, 7))
        print(hot(12000, "ab"))
        print(getattr(hot, "pyopt_compiled", None) is not None)
        """,
    )
    jit_fb = run([PYTHON, "-m", "pyopt", "--jit", str(fallback)])
    if jit_fb.returncode != 0 or jit_fb.stdout != "14\nabab\nTrue\n":
        area.fail(
            ...
            command_text([PYTHON, "-m", "pyopt", "--jit", str(fallback)]),
        )
    ...
    materialized = write_case(
        ...
        print("pyopt-jit" in pathlib.Path(hot.__wrapped__.__code__.co_filename).parts)
        print(pathlib.Path(hot.__wrapped__.__code__.co_filename).exists())
        """,
    )
    mat = run([PYTHON, "-m", "pyopt", "--jit", str(materialized)])
    if mat.returncode != 0 or mat.stdout.splitlines()[-3:] != ["True", "True", "True"]:
        area.fail(
            ...
            command_text([PYTHON, "-m", "pyopt", "--jit", str(materialized)]),
        )
    ...
    s = show(dynamic, "--jit")
    if s.returncode != 0 or "_pyopt_jit" in s.stdout:
        area.fail("JIT injection ran despite remaining dynamic construct", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", "--jit", str(dynamic)]))
    ...
    s = show(cold, "--jit")
    if s.returncode != 0 or "_pyopt_jit" in s.stdout:
        area.fail("JIT injection decorated a cold function", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", "--jit", str(cold)]))
    return area


def verify_benchmark() -> Area:
    area = Area("benchmark")
    bench_cmd = [PYTHON, "-m", "pyopt.bench"]
    ...
    harness = (ROOT / "src" / "pyopt" / "bench" / "__main__.py").read_text(encoding="utf-8")
    ...
    for term in methodology_terms:
        if term not in harness:
            area.fail(f"benchmark methodology check missing {term!r}", str(ROOT / "src" / "pyopt" / "bench" / "__main__.py"))
    return area
...
def verify_constant_propagation() -> Area:
    ...
    if s.returncode != 0 or "print(9)" not in s.stdout or "print('abcd')" not in s.stdout:
        area.fail("fold -> const-prop cascade did not produce folded constants", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(basic)]))
    ...
    if s.returncode != 0 or "print(x)" not in s.stdout or "print(7)" not in s.stdout:
        area.fail("constant propagation changed a use before assignment or missed later use", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(after_only)]))
    ...
    if s.returncode != 0 or "return x" not in s.stdout:
        area.fail("constant propagation entered a nested closure scope", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(nested)]))
    ...
    if s.returncode != 0 or "x + 3" not in s.stdout:
        area.fail("name with multiple bindings was propagated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(multiple)]))
    ...
    if s.returncode != 0 or "x = 4" not in s.stdout or "print(4)" in s.stdout:
        area.fail("module-level propagation ran despite remaining dynamic code", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dynamic)]))
    ...
    if s.returncode != 0 or "print(x)" not in s.stdout:
        area.fail("module constant with global declaration was propagated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(global_decl)]))
    ...
    if s.returncode != 0 or "return x" not in s.stdout:
        area.fail("function constant with nested nonlocal declaration was propagated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(nonlocal_decl)]))
    ...
    if s.returncode != 0 or "len(x)" not in s.stdout:
        area.fail("long string constant was duplicated by propagation", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(long_str)]))
    ...
    if s.returncode != 0 or "print(5)" not in s.stdout or "return 20" not in s.stdout:
        area.fail("constant propagation did not handle flat constant tuple unpacking", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(tuple_unpack)]))
    ...
    if s.returncode != 0 or "x, y = (1, make())" not in s.stdout or "print(x, y)" not in s.stdout:
        area.fail("tuple unpacking with non-constant element was propagated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(tuple_partial)]))
    ...
    if s.returncode != 0 or "a, *rest" not in s.stdout or "print(a, rest)" not in s.stdout:
        area.fail("starred tuple unpacking was propagated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(tuple_star)]))
    return area
...
def verify_licm() -> Area:
    ...
    assert_same_output(area, basic)
    s = show(basic)
    if s.returncode != 0 or "_pyopt_inv_" not in s.stdout or "factor * 7" not in s.stdout or "factor << 3" not in s.stdout or "factor // 5" not in s.stdout:
        area.fail("LICM did not hoist invariant int expressions before loop", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(basic)]))
    elif "acc = acc + factor * 7" in s.stdout or "factor << 3) + factor // 5" in s.stdout:
        area.fail("LICM left original invariant expressions in loop body", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(basic)]))
    ...
    s = show(test_expr)
    if s.returncode != 0 or "_pyopt_inv_" not in s.stdout or "while i < _pyopt_inv_" not in s.stdout:
        area.fail("LICM did not hoist invariant expression from while test", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(test_expr)]))
    ...
    s = show(module_loop)
    if s.returncode != 0 or "_pyopt_inv_" in s.stdout:
        area.fail("LICM hoisted at module level", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(module_loop)]))
    ...
    if s.returncode != 0 or "_pyopt_inv_" in s.stdout:
        area.fail("LICM hoisted expression using name not definitely bound before zero-iteration loop", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(zero_iter_unbound)]))
    ...
    s = show(unsafe_ops)
    if s.returncode != 0 or "_pyopt_inv_" in s.stdout:
        area.fail("LICM hoisted floor-division with non-constant divisor", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(unsafe_ops)]))
    ...
    s = show(loop_mutated)
    if s.returncode != 0 or "_pyopt_inv_" in s.stdout:
        area.fail("LICM hoisted expression using loop-mutated name", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(loop_mutated)]))
    ...
    s = show(dynamic)
    if s.returncode != 0 or "_pyopt_inv_" in s.stdout:
        area.fail("LICM ran inside dynamic function scope", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dynamic)]))
    return area
...
def verify_cse_lencache() -> Area:
    ...
    assert_same_output(area, basic)
    s = show(basic)
    if s.returncode != 0 or "_pyopt_cse_" not in s.stdout or "a * 7" not in s.stdout or "return _pyopt_cse_" not in s.stdout:
        area.fail("CSE did not cache repeated pure int subexpression", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(basic)]))
    ...
    s = show(rebind_span)
    if s.returncode != 0 or "_pyopt_cse_" in s.stdout:
        area.fail("CSE crossed a name rebind between repeated expressions", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(rebind_span)]))
    ...
    s = show(len_loop)
    if s.returncode != 0 or "_pyopt_inv_" not in s.stdout or "len(data)" not in s.stdout or "while i < 4" not in s.stdout:
        area.fail("LICM did not hoist fresh-container len expression out of loop", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(len_loop)]))
    elif "total = total + len(data)" in s.stdout:
        area.fail("fresh-container len expression remained in loop body after LICM", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(len_loop)]))
    ...
    s = show(len_cse)
    if s.returncode != 0 or "_pyopt_cse_" not in s.stdout or "len(data)" not in s.stdout:
        area.fail("CSE did not cache repeated fresh-container len calls in a block", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(len_cse)]))
    ...
    if s.returncode != 0 or "_pyopt_cse_" in s.stdout or "_pyopt_inv_" in s.stdout:
        area.fail("len/cache machinery ran despite module-level len binding", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(bound_builtin)]))
    ...
    s = show(escaped)
    if s.returncode != 0 or "_pyopt_cse_" in s.stdout or "_pyopt_inv_" in s.stdout:
        area.fail("len/cache machinery trusted an escaped mutable container", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(escaped)]))
    ...
    s = show(mutated)
    if s.returncode != 0 or "_pyopt_cse_" in s.stdout or "_pyopt_inv_" in s.stdout:
        area.fail("len/cache machinery trusted a container with subscript write", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(mutated)]))
    ...
    s = show(dynamic)
    if s.returncode != 0 or "_pyopt_cse_" in s.stdout or "_pyopt_inv_" in s.stdout:
        area.fail("len/cache machinery ran in module with remaining dynamic construct", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dynamic)]))
    return area
...
def verify_comprehension_to_map() -> Area:
    ...
    if s.returncode != 0:
        area.fail(f"builtin comprehension rewrite --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(builtins_case)]))
    else:
        for expected in ["sum(map(abs, data))", "sum(filter(bool, data))", "list(map(float, data))", "set(map(str, data))"]:
            if expected not in s.stdout:
                area.fail(f"missing expected comprehension rewrite {expected!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(builtins_case)]))
    ...
    if s.returncode != 0 or "list(map(abs, filter(bool, data)))" not in s.stdout:
        area.fail("map(filter(...)) chain was not generated for f(x) with if g(x)", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(map_filter_case)]))
    ...
    if s.returncode != 0 or "sum(map(f, data))" not in s.stdout:
        area.fail("generator expression with stable module function was not rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(module_fn_gen)]))
    ...
    if s.returncode != 0 or "list(map(f, data))" in s.stdout or "[f(x) for x in data]" not in s.stdout:
        area.fail("list comprehension used module function despite builtins-only rule", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(module_fn_list)]))
    ...
    if s.returncode != 0 or "map(abs, data)" in s.stdout or "(abs(x) for x in data)" not in s.stdout:
        area.fail("generator expression in identity-observable position was rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(gen_identity)]))
    ...
    if s.returncode != 0 or "for y in map(abs, data):" not in s.stdout:
        area.fail("generator expression in for-iter position was not rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(for_iter)]))
    ...
    if s.returncode != 0 or "','.join(map(str, data))" not in s.stdout:
        area.fail("generator expression consumed by literal join was not rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(join_case)]))
    ...
    if s.returncode != 0 or "map(abs, data)" in s.stdout:
        area.fail("comprehension rewrite used shadowed map helper", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(helper_shadow)]))
    ...
    if s.returncode != 0 or "map(abs, data)" in s.stdout:
        area.fail("generator rewrite fired under shadowed consumer builtin", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(consumer_shadow)]))
    ...
    s = show(dynamic)
    if s.returncode != 0 or "map(abs, data)" in s.stdout:
        area.fail("comprehension rewrite ran in module with remaining dynamic construct", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dynamic)]))
    ...
    if s.returncode != 0 or "f(1)" not in s.stdout or "1 + n" in s.stdout:
        area.fail("FunctionInlining ignored comprehension target shadowing of free name", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(inline_shadow)]))
...
def verify_unused_elimination() -> Area:
    ...
    s = show(imports)
    if s.returncode != 0:
        area.fail(f"unused imports --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(imports)]))
    else:
        if "json" in s.stdout or "deque" in s.stdout or "import math" not in s.stdout or "Counter" not in s.stdout:
            area.fail("unused import aliases were not trimmed correctly", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(imports)]))
    ...
    if s.returncode != 0 or "from __future__ import annotations" not in s.stdout or "from math import *" not in s.stdout:
        area.fail("__future__ or import * was removed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(keep_imports)]))
    ...
    s = show(local_assign)
    if s.returncode != 0:
        area.fail(f"unused local assignments --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(local_assign)]))
    else:
        if "pure =" in s.stdout or "impure =" in s.stdout or "side()" not in s.stdout or "return 4" not in s.stdout:
            area.fail("unused local assignments were not removed/downgraded as expected", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(local_assign)]))
    ...
    if s.returncode != 0 or "def add" in s.stdout or "print(5)" not in s.stdout:
        area.fail("unused function helper was not removed after inlining/folding", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(inline_cleanup)]))
    ...
    if s.returncode != 0 or "x = 123" not in s.stdout:
        area.fail("module-level variable assignment was removed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(module_var)]))
    ...
    if s.returncode != 0 or "x = 8" not in s.stdout:
        area.fail("local assignment read by nested closure was removed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(closure)]))
    ...
    if s.returncode != 0 or "def exported" not in s.stdout or "def hidden" in s.stdout:
        area.fail("__all__ did not keep exported function or hidden function survived", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dunder_all)]))
    ...
    if s.returncode != 0 or "def hidden" not in s.stdout:
        area.fail("non-literal __all__ did not disable module-level function removal", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(unknown_all)]))
    ...
    if s.returncode != 0 or "import math" not in s.stdout:
        area.fail("unused elimination ran despite remaining dynamic constructs", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dynamic)]))
    ...
    if s.returncode != 0 or "counter = 1" not in s.stdout:
        area.fail("unused elimination removed a global-declared escaping store", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(global_store)]))
    ...
    if s.returncode != 0 or "counter = 1" not in s.stdout:
        area.fail("unused elimination removed a nonlocal-declared escaping store", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(nonlocal_store)]))
    ...
    if s.returncode != 0 or "import os" not in s.stdout:
        area.fail("unused elimination removed an import bound under global declaration", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(global_import)]))
    return area


def assert_exception(area: Area, path: Path, expected: str) -> None:
    a = run([PYTHON, str(path)])
    b = run([PYTHON, "-m", "pyopt", str(path)])
    if a.returncode == 0 or b.returncode == 0 or expected not in a.stderr or expected not in b.stderr:
        area.fail(
            ...
            f"  pyopt    rc={b.returncode} stderr={b.stderr!r}",
            command_text([PYTHON, str(path)]),
            command_text([PYTHON, "-m", "pyopt", str(path)]),
        )
...
def verify_greatest_fixpoint_analysis() -> Area:
    ...
    if s.returncode != 0 or "i * 1" in s.stdout or "acc << 0" in s.stdout:
        area.fail("self-referential int accumulator identities did not simplify", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(acc)]))
    ...
    if s.returncode != 0 or "x + 0" not in s.stdout:
        area.fail("float branch contamination did not preserve x + 0", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(branch_float)]))
    ...
    s = show(branch_call)
    if s.returncode != 0 or "x + 0" not in s.stdout:
        area.fail("call-result contamination did not preserve x + 0", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(branch_call)]))
    ...
    if s.returncode != 0 or "x * 1" not in s.stdout:
        area.fail("float aug-assign contamination simplified x * 1", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(div_contam)]))
    ...
    s = show(bool_case)
    if s.returncode != 0 or "x * 1" not in s.stdout:
        area.fail("bool binding was treated as plain int", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(bool_case)]))
    ...
    s = show(try_case)
    if s.returncode != 0 or "x + 0" not in s.stdout:
        area.fail("try/except non-int rebinding did not preserve x + 0", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(try_case)]))
    return area
...
def verify_dedynamize_eval() -> Area:
    ...
    if s.returncode != 0 or "eval" in s.stdout or "abcd" not in s.stdout:
        area.fail("module-level eval constant expression was not inlined/folded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    if s.returncode != 0 or "eval(" not in s.stdout or "'ok'" not in s.stdout or "eval(code)" not in s.stdout:
        area.fail("non-constant eval did not roll back all eval rewrites", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(rollback)]))
    ...
    if s.returncode != 0 or "eval('(x:=5)')" not in s.stdout:
        area.fail("eval string containing walrus was expanded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(walrus)]))

    syntax = write_case("dedyn_eval_syntax", "print(eval('1+'))\n")
    a = run([PYTHON, str(syntax)])
    b = run([PYTHON, "-m", "pyopt", str(syntax)])
    if a.returncode == 0 or b.returncode == 0 or "SyntaxError" not in a.stderr or "SyntaxError" not in b.stderr:
        area.fail(
            ...
            command_text([PYTHON, "-m", "pyopt", str(syntax)]),
        )
    s = show(syntax)
    if s.returncode != 0 or "eval('1+')" not in s.stdout:
        area.fail("eval syntax-error string was expanded or removed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(syntax)]))
    ...
    if s.returncode != 0 or "eval('x'" not in s.stdout:
        area.fail("2-arg eval was expanded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(multiarg)]))
    multiarg3 = write_case("dedyn_eval_multiarg3", "print(eval('x', {'x': 7}, {}))\n")
    assert_same_success_output(area, multiarg3)
    s = show(multiarg3)
    if s.returncode != 0 or "eval('x'" not in s.stdout:
        area.fail("3-arg eval was expanded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(multiarg3)]))
    return area
...
def verify_dedynamize_exec() -> Area:
    ...
    if s.returncode != 0 or "exec" in s.stdout or "a = 2" not in s.stdout or "b = 5" not in s.stdout or "print(2, 5)" not in s.stdout:
        area.fail("module-level exec constant statements were not spliced and optimized", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    if s.returncode != 0 or "exec('x = 2')" not in s.stdout:
        area.fail("exec inside function was expanded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(in_func)]))
    ...
    if s.returncode != 0 or "exec" in s.stdout or "print('done')" not in s.stdout:
        area.fail("exec('') inside if-block was not safely removed by later passes", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(empty_if)]))
    ...
    if s.returncode != 0 or "exec('from __future__ import annotations')" not in s.stdout:
        area.fail("exec string containing __future__ import was expanded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(future)]))
    return area
...
def verify_dedynamize_globals_vars() -> Area:
    ...
    if s.returncode != 0 or "globals()" in s.stdout or "vars()" in s.stdout or "x = 2" not in s.stdout or "y = 5" not in s.stdout or "print(2, 5)" not in s.stdout:
        area.fail("module-level globals/vars writes were not rewritten and optimized", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    if s.returncode != 0 or "globals()['x']" not in s.stdout or "globals()['x'] = 1" not in s.stdout:
        area.fail("globals read was rewritten or write did not roll back with remaining dynamic read", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(read)]))
    ...
    if s.returncode != 0 or "globals()['x'] = 4" not in s.stdout:
        area.fail("function-scope globals write was expanded", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(in_func)]))
    return area
...
def verify_dedynamize_attributes() -> Area:
    ...
    if s.returncode != 0 or "getattr" in s.stdout or "setattr" in s.stdout or "delattr" in s.stdout or "o.x = 5" not in s.stdout or "del o.x" not in s.stdout or "print(o.x)" not in s.stdout:
        area.fail("getattr/setattr/delattr family was not rewritten to attribute syntax", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    for name, src in blocked_cases.items():
        ...
        s = show(case)
        if s.returncode != 0:
            area.fail(f"{name}: --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
            continue
        if name == "dedyn_getattr_default" and "getattr(o, 'a', 9)" not in s.stdout:
            area.fail("3-arg getattr was rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
        if name == "dedyn_getattr_keyword" and "getattr(o, 'class')" not in s.stdout:
            area.fail("keyword attribute string was rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
        if name == "dedyn_getattr_nonidentifier" and "getattr(o, 'not-ident')" not in s.stdout:
            area.fail("non-identifier attribute string was rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
        if name == "dedyn_setattr_call_obj" and ("setattr(" not in s.stdout or ".x =" in s.stdout):
            area.fail("setattr with call object was rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
        if name == "dedyn_getattr_rebound" and "getattr(1, 'real')" not in s.stdout:
            area.fail("getattr was rewritten despite module rebinding", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
        if name == "dedyn_getattr_param" and "getattr(1, 'x')" not in s.stdout:
            area.fail("getattr was rewritten despite function parameter rebinding", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
        if name == "dedyn_exec_binds_getattr" and ("exec(" not in s.stdout or "getattr(o, 'x')" not in s.stdout):
            area.fail("exec binding getattr did not abort family rewrites", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(case)]))
    return area
...
def verify_dedynamize_cascade_rollback() -> Area:
    ...
    if s.returncode != 0 or "exec" in s.stdout or "globals()" in s.stdout or "add(" in s.stdout or "x = 3" not in s.stdout or "print(7)" not in s.stdout:
        area.fail("successful de-dynamize did not unlock folding and inlining in the same run", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    s = show(rollback)
    if s.returncode != 0:
        area.fail(f"rollback purity --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(rollback)]))
    else:
        ...
        for expected, msg in checks:
            if expected not in s.stdout:
                area.fail(msg, command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(rollback)]))
    return area
...
def verify_constant_folding() -> Area:
    ...
    s = show(p)
    if s.returncode != 0:
        area.fail(f"{p.name}: --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    else:
        if "1 / 0" not in s.stdout:
            area.fail("1/0 was folded or rewritten away", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        if "1 < 'a'" not in s.stdout and '1 < "a"' not in s.stdout:
            area.fail("1<'a' was folded or rewritten away", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    huge = write_case("huge_literal_guard", "print(10 ** 10 ** 10)\n")
    try:
        s = subprocess.run(
            [PYTHON, "-m", "pyopt", "--show", "--no-run", str(huge)],
            cwd=ROOT,
            env=env(),
            text=True,
            capture_output=True,
        ...
        area.fail("huge literal guard did not complete quickly", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(huge)]))
    else:
        if s.returncode != 0:
            area.fail(f"huge literal guard --show failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(huge)]))
        elif "**" not in s.stdout:
            area.fail("outer power was fully evaluated instead of preserved", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(huge)]))
        elif any(len(m.group(0)) > 1300 for m in re.finditer(r"\d+", s.stdout)):
            area.fail("optimized source emitted an integer literal longer than 1300 digits", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(huge)]))
    return area
    ...
    if s.returncode != 0:
        ...
        for absent in ["bad-if-false", "bad-if-true-else", "bad-while", "after", "assert True"]:
            if absent in s.stdout:
                area.fail(f"{absent!r} survived dead-code elimination", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        for present in ['"""module doc"""', '"""function doc"""']:
            if present not in s.stdout and present.replace('"""', "'") not in s.stdout:
                area.fail(f"{present} docstring not preserved in optimized source", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    return area
...
def verify_algebraic() -> Area:
    ...
    if s.returncode != 0:
        ...
        if "'x' + 0" not in s.stdout and '"x" + 0' not in s.stdout:
            area.fail("non-int string addition was simplified despite not being a provable int", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        if "Weird() + 0" not in s.stdout:
            area.fail("custom object addition was simplified despite not being a provable int", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        if "x + 0" in s.stdout or "0 + x" in s.stdout or "x * 1" in s.stdout:
            area.fail("provable int identities were not simplified", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    return area
...
def verify_inlining() -> Area:
    ...
    if s.returncode != 0 or "f(x, 3)" in s.stdout or "f(1, y)" in s.stdout:
        area.fail("safe module-level f(a,b) calls were not inlined", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    if s.returncode != 0 or "f(side())" not in s.stdout:
        area.fail("call with side-effect argument was inlined or --show failed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(side_blocked)]))
    ...
    if s.returncode != 0 or "f(1)" not in s.stdout:
        area.fail("rebound function was inlined or --show failed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(rebound_blocked)]))
    ...
    if s.returncode != 0 or "f(1)" not in s.stdout:
        area.fail("call before def was inlined or --show failed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(before_def_blocked)]))
    ...
    if s.returncode != 0 or "f(2)" not in s.stdout:
        area.fail("module containing eval still inlined a function call or --show failed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(dynamic_blocked)]))
    return area
...
def verify_localize_regressions() -> Area:
    ...
    assert_same_output(area, basic)
    s = show(basic)
    if s.returncode != 0 or s.stdout.count("_pyopt_glb_") < 3 or " = abs" not in s.stdout or " = min" not in s.stdout or " = helper" not in s.stdout:
        area.fail("localize did not bind abs/min/module helper to _pyopt_glb temps", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(basic)]))
    ...
    if s.returncode != 0 or " = abs" in s.stdout:
        area.fail("localize bound abs despite a shadowing binding elsewhere", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(shadowed_builtin)]))
    ...
    for name, src in blocked_cases.items():
        ...
        assert_same_output(area, p)
        s = show(p)
        if s.returncode != 0 or "_pyopt_glb_" in s.stdout:
            area.fail(f"{name}: localize introduced a temp for a blocked case", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
...
def verify_inline_statement_regressions() -> Area:
    ...
    assert_same_output(area, basic)
    s = show(basic)
    if s.returncode != 0 or "_pyopt_in_" not in s.stdout or "helper(2, 3)" in s.stdout:
        area.fail("statement-body inlining did not introduce _pyopt_in temps and remove the call", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(basic)]))
    ...
    if s.returncode != 0 or s.stdout.count("arg(1)") != 1 or s.stdout.count("arg(2)") != 1:
        area.fail("statement-body inlining did not preserve one left-to-right evaluation of side-effecting args", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(side_effects)]))
    ...
    for name, src in blocked_cases.items():
        ...
        s = show(p)
        if s.returncode != 0:
            area.fail(f"{name}: --show failed", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        elif name == "inlinestmt_keyword_blocked" and "helper(a=2)" not in s.stdout:
            area.fail("keyword call was statement-inlined", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        elif name == "inlinestmt_unboundlocal_preserved" and "helper()" not in s.stdout:
            area.fail("read-local-before-assign helper was statement-inlined", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        elif name == "inlinestmt_expression_positions_blocked" and ("helper(2)" not in s.stdout or "helper(total)" not in s.stdout):
            area.fail("expression-position statement-body call was rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        elif name == "inlinestmt_shadowed_free_name" and "helper(v)" not in s.stdout:
            area.fail("helper with shadowed free name was statement-inlined", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        elif name == "inlinestmt_oversized_body" and "helper(1)" not in s.stdout:
            area.fail("helper with 7 assigns was statement-inlined", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
...
def verify_strength_interval_regressions() -> Area:
    ...
    if s.returncode != 0 or "i & 7" not in s.stdout or "i >> 2" not in s.stdout or "i * i" not in s.stdout:
        area.fail("power-of-two strength rewrites did not fire for proven int range target", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(strength)]))
    ...
    if s.returncode != 0 or "** 2" not in s.stdout:
        area.fail("pow-2 duplication ignored the node cap", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(pow_cap)]))
    ...
    s = show(abs_cases)
    if s.returncode != 0 or "total += i" not in s.stdout or "_pyopt_glb_" not in s.stdout:
        area.fail("abs interval rewrite did not remove only the non-negative range-target case", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(abs_cases)]))
    ...
    if s.returncode != 0 or "total += i" in s.stdout:
        area.fail("shadowed abs was eliminated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(shadow_abs)]))
    ...
    if s.returncode != 0 or "i + 0" in s.stdout or "i * 1" in s.stdout or "i & 7" not in s.stdout or "i >> 2" not in s.stdout or "abs(i)" in s.stdout:
        area.fail("range(n)/range(True)/__index__ int-target rewrites did not fire as expected", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(range_args)]))
    ...
    if s.returncode != 0 or "return 'xy'" in s.stdout:
        area.fail("range target reused as str was treated as a stable int after the loop", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(reused_as_str)]))
...
def verify_cross_scope_constprop() -> Area:
    ...
    if s.returncode != 0 or "if DEBUG" in s.stdout or "'debug'" in s.stdout or '"debug"' in s.stdout:
        area.fail("cross-scope DEBUG branch was not eliminated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(branch)]))
    ...
    if s.returncode != 0 or "return DEBUG" not in s.stdout:
        area.fail("def-before-binding module constant was propagated", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(def_before)]))
    ...
    if s.returncode != 0 or "print(True)" not in s.stdout:
        area.fail("local shadow did not preserve local constant propagation semantics", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(local_shadow)]))
    elif "def f():\n    return False" in s.stdout:
        area.fail("module constant propagated through a local shadow", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(local_shadow)]))
    ...
    for name, src in disabled.items():
        ...
        if s.returncode != 0 or (name != "constprop_cross_comp_target_shadow" and "DEBUG" not in s.stdout):
            area.fail(f"{name}: --show failed or lost DEBUG name unexpectedly", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        if name == "constprop_cross_comp_target_shadow" and "for DEBUG in" not in s.stdout:
            area.fail("comprehension target shadow was substituted", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    ...
    if s.returncode != 0 or "body = VALUE" in s.stdout or "return VALUE" in s.stdout or "lambda: VALUE" in s.stdout:
        area.fail("nested def/class method/lambda did not receive cross-scope module constant", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(nested)]))
    return area
...
def verify_dynamic_fallback() -> Area:
    ...
    for name, src in samples.items():
        ...
        s = show(p)
        if s.returncode != 0:
            area.fail(f"{name}: --show --no-run failed: {s.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
            continue
        original = p.read_text(encoding="utf-8").strip()
        shown = s.stdout.strip()
        if "1 + 2" not in shown:
            area.fail(f"{name}: dynamic scope was optimized instead of left untouched\nshown={shown!r}", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
        if name == "dyn_star" and "from math import *" not in shown:
            area.fail("import * dynamic file was unexpectedly rewritten", command_text([PYTHON, "-m", "pyopt", "--show", "--no-run", str(p)]))
    return area
...
def verify_cli() -> Area:
    ...
    cp = run([PYTHON, "-m", "pyopt", "--no-run", "--show", "--report", "-o", str(out), str(p)])
    if cp.returncode != 0:
        area.fail(f"combined CLI flags failed: {cp.stderr!r}", command_text([PYTHON, "-m", "pyopt", "--no-run", "--show", "--report", "-o", str(out), str(p)]))
    else:
        if "print('argv', sys.argv[1:])" not in cp.stdout and 'print("argv", sys.argv[1:])' not in cp.stdout:
            area.fail("--show did not print optimized source", command_text([PYTHON, "-m", "pyopt", "--no-run", "--show", str(p)]))
        if "constant-folding" not in cp.stderr or "pyopt:" not in cp.stderr:
            area.fail("--report did not include pass stats on stderr", command_text([PYTHON, "-m", "pyopt", "--no-run", "--report", str(p)]))
        if not out.exists() or "print" not in out.read_text(encoding="utf-8"):
            area.fail("-o did not write optimized source", command_text([PYTHON, "-m", "pyopt", "--no-run", "-o", str(out), str(p)]))
    return area
