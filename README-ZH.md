...
`int` 注解说的是"是整数",从不说"不大",所以从它推不出任何区间——这就是注解参数单靠自己**不会**被标注的原因。开了 `--aggressive=annotations` 之后它有第二条路:函数里只要有一个 `int` 注解参数,整个函数切到 **checked 模式**——里面每个可证 int 都被标注(不管有没有界),并给该函数加上 `@cython.overflowcheck(True)`。
