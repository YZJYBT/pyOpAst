...
诚实的实测:21 个负载、best-of-7、两边都由 Cython 编译,`--cython` 相对同一份源码的原味 cythonize 是 **0.93x geomean**——是**变慢**。因为 Cython 在若干行上的速度,正是来自上面那个算错答案的推断(被误编译的推导式比正确版本快 1.55x)。而标注本身目前覆盖极低:按下面的"全有或全无"规则,52 个基准函数里只有 2 个被标注——**部分标注是负收益**,typed 值遇到 untyped 操作数要装箱回 PyObject 再拆箱,实测 0.72x(循环计数器标了、累加器没标)。
