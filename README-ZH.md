...
默认层产出的是不带任何自有运行时的普通 Python 源码,因此它不是"更快解释器"的替代品,而是可以叠在其上。把默认层产物交给 **PyPy 7.3.21** 跑(对照同一份原始源码),同样 21 个负载上 geomean **1.98x**(best-of-3、每次独立进程)。收益来自 PyPy 的追踪 JIT 不会替你做的那些改写——去动态化、优化期循环折叠、推导式与外提类重写;少数负载会略慢,因为 JIT 有时更喜欢你原本写的形态。`--jit` 依赖 numba,仅限 CPython,PyPy 上请停在默认层。
