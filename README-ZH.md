...
| 条件事实收窄 `cond-narrow` | 用区间知识判定 proven-int 表达式之间的比较,能确定结果的直接换成 `True`/`False`,交给下游 `dead-code` 删分支——`for i in range(n)` 里的 `if i < 0:` 由此整块消失。两类知识合流:① **流不敏感基础区间**(`infer_int_ranges`,全部绑定的并 + 加宽),这是"range 目标必非负"的来源;② **路径敏感收窄**——进入 `if c:` 时该条件成立,故其中被比较的名字在该块内携带更紧的区间,`if k > 10: ... if k > 5:` 的内层由此判真;`and` 在真分支收窄、`or` 在假分支收窄、`not` 互换。此外做**赋值传递**:`x = <int 表达式>` 后按当前环境重算 x 的区间(右值先于存储求值,故用语句前的环境),比退回基础区间锐利得多。**健全性**:任何语句执行后,其子树内绑定过的名字一律**重置为基础区间**(基础区间是该名字任何时刻取值的上界,重置永远安全);进入循环体先重置循环子树内的全部绑定(循环体也会在此前迭代改动它们之后运行),再施加循环测试这一事实。只折叠**纯** int 表达式(名字/int 常量/算术/新鲜容器的 `len()`),故删除求值不可能删掉副作用;链式比较(`a < b < c`)不处理;仅函数作用域(模块全局可被调用的函数改写)。裸真值测试也参与:区间不含 0 的名字恒真,固定为 `[0, 0]` 的恒假。 |
