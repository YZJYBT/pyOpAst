...
opast 能优化**它自己**(`python -m opast.build src/opast -o out/opast`):57 个文件 841 处改写、零回退——自优化后的副本对全部基准语料产出与原版**字节级相同**的优化结果,并通过完整验收套件,即编译器界经典的 bootstrap 校验。真正使用 `exec`/`compile` 的模块(runner、import hook)按动态代码规则整域跳过,与文档所述完全一致。诚实的脚注:自优化后的优化器**并没有变快**(0.98x,噪声内)——opast 自己的热路径是方法密集的 visitor 类,恰好是证明背书层刻意不碰的领域。语义保持经受住了它最严苛的用户;加速收益属于形如上表基准的代码。
