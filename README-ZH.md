...
| 模式 | geomean 加速 |
| --- | --- |
| 普通(仅证明背书的默认层,产物是纯 CPython 源码) | **2.23x** |
| 激进 `-O3`(全部假设背书选项,含 numba jit) | **18.7x** |

个别行:常量 `eval`/`getattr` 去动态化 **34.5x**、内联→折叠级联 **2.4x**(`-O3` 下 50x)、循环不变量外提 **1.9x**、日常风格结算报表脚本 **1.3x**;numba 能整体接管的数值核在 `-O3` 下达 **50–1200x**。`control` 负载无可优化项、**0 改写**——它在各轮之间 0.91x–1.03x 的抖动是测量噪声本底,不是效应。

### 与 PyPy 可叠加

默认层产出的是不带任何自有运行时的普通 Python 源码,因此它不是"更快解释器"的替代品,而是可以叠在其上。把默认层产物交给 **PyPy 7.3.21** 跑(对照同一份原始源码),同样 21 个负载上 geomean **1.98x**(best-of-3、每次独立进程)。收益来自 PyPy 的追踪 JIT 不会替你做的那些改写——去动态化、优化期循环折叠、推导式与外提类重写;少数负载会**变慢**(实测最差 0.62x),因为 JIT 有时对你原本写的形态追踪得更好。`--jit` 依赖 numba,仅限 CPython,PyPy 上请停在默认层。
...
| `numpy` | **numpy 向量化**:不动点之后,推导式 map(`acc = [f(v) for v in xs]`)与 `+=` 归约——迭代对象须为可证新鲜的列表/元组或 `range(...)`——被提取为生成的辅助函数对(逐字原样的 Python 版 + numpy 版),由运行期分发器接管:首调两版对照校验,任何不匹配、异常、不支持的 dtype(含异构列表)或 numpy 缺席都使该调用点**永久回退** Python 路径;除零会重新抛出与原代码完全一致的异常。结果经 `.tolist()`/`.item()` 返回,元素类型保持精确的 Python int/float。实测(best-of-7):range 归约 **8.8–23.8x**、range map ~1.5x、重元素(≥4 运算)列表 map/归约 ~2–3x;轻元素列表形态实测 0.61–0.92x,已按此静态跳过 | 数值循环的中间值变为定宽 int64/float64(大整数回绕而非增长)、浮点归约被重结合为一次部分和、numpy 须可导入才有快路径(否则全程纯 Python) |
| `opt-imports` | 上文的导入钩子 | 被优化的导入模块不会被 monkeypatch |
| `pure-calls` | **可信纯函数**:你自己的辅助函数调用成为 LICM/CSE/unused 的操作对象——实参循环不变的调用提到循环外、重复调用只算一次、结果无人用的调用整条消失,并与内联/属性外提级联。信任基于证据、在不动点循环前对原始模块一次性判定:模块顶层 `def`、名字全模块只绑定一次(没有 `f = wrap(f)`、没有 `f.attr = ...`)、默认参数不可变,且函数体无嵌套作用域、`yield`/`await`、`global`、`import`、`with`、属性访问、下标写入,只读取自身局部变量、其他可信函数、白名单纯内建与不可变模块常量(互递归经不动点收敛) | 可信函数是纯的且运行期不被重绑:其调用可能在不同时机执行或干脆不执行(其中的异常与不终止可能移动或消失)、同参数调用可能共享同一个结果对象、调用方在被优化区域内不改动实参对象 |
...
- **`loopfold`(数千倍)是构造出的极端案例**,不是典型收益:整个计算发生在**优化期**(常量循环闭式折叠),运行期脚本只剩常量赋值。真实的账是"优化期一次性付 ~0.13s,之后每次运行省 ~0.23s"。`dedynamize`(34.5x)同理——热循环里的常量 `eval`/`getattr` 坍缩为静态代码。
- **中间行才是代表性数字**:`mixed` 2.38x、`comptomap` 2.34x、`algebra` 2.19x、`licm` 1.88x、`condnarrow` 1.60x、`daily`(一次触及多数 pass 的真实风格订单结算报表)1.30x。
...
- **numba 白名单**:仅数值常量与 int/float 算术、元组(打包/解包/多返回值/常量下标读取——numba 的 static_getitem 对异构元组也能类型化)、`if`/`while`/`for range()`/`return`、调用仅限 `range/abs/min/max/round/divmod/int/float/bool/len`、`math.*` 与**其他白名单候选函数**(见下)、名字仅限参数、局部变量与**可证单绑定的数值/元组模块常量**(numba 把全局冻结为编译期值,单绑定证明让冻结精确无误);模块存在单绑定的 `import numpy` 时,候选还可调用白名单内的 `np.*` 函数、做变量下标与切片读取、读 `.size`/`.shape`/`.ndim`、`raise` 常量消息的内建异常、并对可证由 `np.zeros`/`ones`/`empty_like` 等创建的局部数组做下标写入——数组型内核整函数可编译;开启激进 `annotations` 时,标注为 `np.ndarray`/`npt.NDArray`(含模块级别名)的参数同样视为已证数组,就地算法(排序、划分)由此解锁——此类**变异实参**的候选跳过首调对照(对同一参数重跑就地函数本身不健全),只依赖白名单与 numba 类型检查;lazy 候选现可互调:同伴别名在触发时回填,变量边界的归并内核连同 `_gallop_*` 辅助函数整体编译(200k 元素排序基准实测:归并稳态 5.3x、三路快排 17.8x);**`DynArray`**(`from opast import DynArray`)是普通的 `list` 子类——在 jit 函数之外、以及任何没跑过 opast 的解释器里,它**就是 list**;用它等于声明一条静态分析证不出的契约:*元素是同构数值、没有代码依赖容器的标识或具体类型、且它不离开创建它的函数*。在被编译的函数内部,这条契约让 opast 在**编译副本里**换用 numba 最快的表示(Python 回退始终跑原来的 list,编译失败或没装 numba 都逐字正确):`DynArray()` + `append`/`pop` → numba 列表(`[]`,数组给不了的动态容量),400k 单调栈实测 **32x**;`DynArray.zeros(n)`/`.full(n, v)` → 模块导入了 numpy 时换 `np.zeros(n)`/`np.full(n, v)`,否则仍是列表,100 万次更新的直方图实测 **37x**。`dtype` 显式给出(`zeros(n)` → `0.0`/float64,`zeros(n, dtype=int)` → `0`/int64),保证两种表示逐元素一致。优化器验证契约中机械可查的一半——单次绑定、不返回、不外传、不别名、增长方法只许用在可增长形态上、真值测试在副本里改写为 `len(x) > 0`——任何其他触碰都会让整个函数出局。**每轮新建又丢弃的容器不值得**(重建 3 万次的短命栈实测 1.07x,分配成本占满),让它活过整个热循环;`--jit` 下即使模块里没有别的代码读 numpy/math,这两个导入也会被保留——上面那条改写正是它们的读者;无字符串、无 list/dict/set(反射容器每次调用整表复制且已弃用)、无属性(math/numpy 根除外)/闭包/try/yield;
...
## 实验性:`--cython`(先保正确,能证明的地方才快)

```powershell
opast -O2 --cython -o build/hot.py hot.py
cythonize -i build/hot.py                  # 编译是你自己的构建步骤
```

`--cython` 往优化后的源码里发射 Cython 纯 Python 模式标注:**能证明**的名字加 `@cython.locals(...)`,并给每个函数加 `@cython.infer_types(False)`。产物仍是普通 Python(带一段有守卫的 import 垫片,没装 Cython 也照跑),只有你接着去编译它才有意义。

它最初是为修正确性而做的,而这一半仍然最重要。**原味 `cythonize` 并不保持语义**,而且两处偏差都能在本仓库自己的基准语料里复现:

| 正确的 Python | 原味 Cython | opast `--cython` |
| --- | --- | --- |
| `[j * j % 97 for j in range(0, 400000, 4)]` 末元素 `54` | **`20`**——它给 `j` 推断了 C `long`,MSVC 下只有 32 位,`399996²` 回绕 | `54` |
| 读条件绑定的局部变量抛 `UnboundLocalError` | **返回 `0`**——C 变量没有"未绑定"状态 | 照常抛 |

两者都出自 Cython 自己的"安全"类型推断:它没有区间分析可查,而 opast 有(`j * j` 可证落在 `(0, 159996800016)`,需要 64 位,所以 `j` 不能是 32 位 `long`)。**关掉那个推断**才能把语义还回来,再把 opast 能证明的类型显式补上。

速度方面,21 个负载、两侧都由 Cython 编译:**1.29x geomean、中位 1.05x**(best-of-15)。但这个平均值不是"普涨",而是**两个大赢**撑起来的——覆盖仍然很窄——基准语料里的 36 个函数只有 4 个拿到类型——多数行在 1.0x 附近,收益集中在"整个热函数能被端到端标注"的地方:

| | `-O2` + 原味 cythonize | `--cython` | |
| --- | --- | --- | --- |
| `annotated` | 22.7 ms | **1.4 ms** | **16.6x** |
| `strength` | 163.4 ms | **11.1 ms** | **14.8x** |

(交错 best-of-13,结果一致,按中位数算是 17.0x 与 11.7x——不是 min-of-N 的假象。)

但**小数字要当心**:这里的计时单位是"一个进程里的一次模块导入"(CPython 扩展模块无法在同一进程内重复执行),Windows 起进程会带首次接触文件的扫描尖峰,best-of-N 压不干净。1.0x 附近的行在两轮之间能差 ±0.4x;全量那轮报 0.71x、0.72x 的两行,单独重测是 1.04x 和 1.01x。只有上表那个量级的差异才值得读。

不依赖这套计时也站得住的部分:

- 没拿到类型的行,`--cython` 与 `-O2` 产物的差别**只有那道指令**,那里的损失按构造就是正确性的价钱;单负载四版本对照把两半分开计价:**指令要价 0.74–0.80x**;
- **部分标注比不标注更差**——typed 值遇到 untyped 操作数每次都要装箱回 PyObject 再拆箱,实测 0.72x(循环计数器标了、累加器没标)。这正是下面"全有或全无"规则的由来,也是覆盖窄的原因:opast 拒绝标注的次数远多于标注;
- **具体哪几个函数合格取决于开了哪些选项**:这个 pass 跑在最后,看到的是前面各 pass 留下的形状。同样 21 个负载,`-O2` 下标注 4 个、裸 `--aggressive=annotations` 下也是 4 个,但**不是同 4 个**(前者含 `fastmath.work`,后者含 `tailrec.work`);一个激进项都不开时是 2 个;
- 每一轮里最差的行都是那个被误编译的推导式(0.49x–0.61x)——**算错的那份确实比算对的快一倍左右**。

一个名字要被标注需同时满足:可证 float(Python float 本身就是 C double)或可证 int 且区间落在 int64 内;每次读取前都已确定绑定(**必须有单个绑定点支配全部使用**,在 `if` 两个分支里各赋值一次的名字不合格);未被嵌套作用域或推导式捕获;没有 `del`、`global`/`nonlocal`、没有参与 `is` 比较;并且它在算术里遇到的每个操作数也都被标注。距离天花板的差距在**区间分析的覆盖**,不在机制本身——把 opast 拒绝标注的那个循环的 5 个局部变量全部手工标上,实测 **11.35x**。

### 有注解的整数:checked 模式

`int` 注解说的是"是整数",从不说"不大",所以从它推不出任何区间——这就是被声明的名字单靠自己**不会**被标注的原因。开了 `--aggressive=annotations` 之后它有第二条路:只要有一个被信任的 `int` 推不出区间,它所在的函数就切到 **checked 模式**——里面每个可证 int 都被标注(不管有没有界),并给该函数加上 `@cython.overflowcheck(True)`。

**参数和局部变量都被信任**,`x: int = compute()` 也算——但局部变量必须满足"该声明是这个名字的**唯一**绑定":名字若在别处被重绑(循环目标、第二次赋值),它可能装着注解从未描述过的东西,而运行期没有任何东西会去复查。信任还必须**参与不动点**而不只覆盖被声明的那个名字:`acc = (acc + i * x) % M` 要先证得出 `x` 是 int,`acc` 才是 int——否则一个不透明但有声明的局部变量,会通过"全有或全无"规则把整个本可标注的函数拖垮。

```python
def b(): x: int = math.floor(2.5)   # 信任 → 进 checked 模式,x/acc/i 全标
def c(): x: float = math.sqrt(2.0)  # 信任 → 标 double,不需要检查
def d(): x: int = 5                 # 可证有界 → 标注,不需要检查
def e(): x: int = f(); …; for x in r: …   # 不是唯一绑定 → 一个都不标
```

**注解撒谎会响,但不保证与 Python 一致**。`s: int = str(n)` 编译后在**声明处**抛 `TypeError`——实测两种情形都如此:一种是 Python 自己后续也会绊倒的谎,另一种是 **Python 根本不会发现的谎**(函数只用 `len(s)`:纯 Python 返回 `4953`,编译后抛 `TypeError`)。这正是 `annotations` 假设本身在起作用——你断言了类型,断言为假时你拿到异常而不是错答案——但它确实意味着**一个 Python 能容忍的注解,在这里会变成错误**。

这道指令是这条路能成立的全部理由,因为 C 整数出错的两种方式**响度完全不同**:参数转换时装不下 int64,Cython **已经**会抛 `OverflowError`(边界是检查的);但 C 整数之间的**算术**不检查——typed 函数里 `4e9 * 4e9` 实测得到 `-2446744073709551616`。`overflowcheck` 让后者也抛,代价约 0.79x。于是这个赌注读作:**这些值放得下 64 位,放不下时你拿到的是异常而不是错误答案**——比 `--jit` 对同一类算术给出的契约(int64 静默回绕)严格更响。

单个注解函数实测(300 万次迭代,best-of-11,两侧都由 Cython 编译):

```
def hot(rounds: int, seed: int) -> int:      -O2 + 原味 cythonize    310.07 ms
...
`hot(3_000_000, 7)` 与纯 Python 逐字节一致;而 `seed=4e9`(Python 会用大整数算出来)在这里抛 `OverflowError`。
