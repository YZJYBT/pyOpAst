...
它存在的理由不是速度。**原味 `cythonize` 并不保持语义**,而且两处偏差都能在本仓库自己的基准语料里复现:
...
速度方面,21 个负载、两侧都由 Cython 编译:**best-of-7 是 0.93x geomean,best-of-15 是 0.97x**(中位数两次都是 0.98x)——大致在盈亏线附近,损失集中在 Cython 的推断原本确实在干活的那几行。这两个数请当**区间**看而不是测量值:这里的计时单位是**一个进程里的一次模块导入**(CPython 扩展模块无法在同一进程内重复执行),而 Windows 上起进程会带首次接触文件的扫描尖峰,best-of-N 压不干净,逐行数字在两轮之间能差 ±0.4x。

不依赖这套计时也站得住的部分:

- **21 个负载里有 19 个一个 `cython.locals` 都没拿到**,它们与 `-O2` 产物的全部差别就是那道指令。那里的损失按构造就是正确性的价钱;
- 对单个负载做四版本对照(交错 11 轮:`-O2` / 只加指令 / 只加类型 / 两者都加)可以把两半分开计价:**指令要价 0.74–0.80x,opast 的类型在能用的地方值 1.73x**;
- 两轮全量里最差的行都是那个被误编译的推导式(0.59x、0.49x)——**算错的那份确实比算对的快一倍左右**。

标注覆盖目前很低:按下面的"全有或全无"规则,52 个基准函数里只有 2 个被标注——**部分标注是负收益**,typed 值遇到 untyped 操作数每次都要装箱回 PyObject 再拆箱,实测 0.72x(循环计数器标了、累加器没标)。

一个名字要被标注需同时满足:可证 float(Python float 本身就是 C double)或可证 int 且区间落在 int64 内;每次读取前都已确定绑定;未被嵌套作用域或推导式捕获;没有 `del`、`global`/`nonlocal`、没有参与 `is` 比较;并且它在算术里遇到的每个操作数也都被标注。距离天花板的差距在**区间分析的覆盖**,不在机制本身——把 opast 拒绝标注的那个循环的 5 个局部变量全部手工标上,实测 **11.35x**。
